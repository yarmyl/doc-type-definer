...
def erode(img):
    image = cv2.erode(img, np.ones((3, 3), np.uint8), iterations=1)
    return image
...
#    image = erode(image)
    ...
    for idx in range(len(contours)):
        ...
        if r > 0.4 and w > 8 and h > 8:
            ...
            if w > h*2:
                i += 1
                def_img = image[y:y+h-1, x:x+w-1]
                cv2.imwrite('temp/'+str(i)+'.png', image[y:y+h-1, x:x+w-1])
...
#                           config='--psm 6 --oem 1',
                ...
                i += 1
                def_img = turn(image[y:y+h-1, x:x+w-1], -90)
                cv2.rectangle(save_img, (x, y), (x+w-1, y+h-1), (0, 255, 0), 2)
                cv2.imwrite('temp/'+str(i)+'.png', def_img)
                print(pytesseract.image_to_string(
                            def_img,
                            lang='rus',
                           config='--psm 6 --oem 1',
